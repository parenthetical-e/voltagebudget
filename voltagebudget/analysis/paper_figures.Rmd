...
```{r, message=FALSE, fig.width=1.5, fig.height=1.3}
name <- "../data/exp7_d-2e-3_f20.csv"
...
name <- "../data/exp5_d-2e-3_f8_r1.csv"
...
name <- "../data/exp5_d-2e-3_f8_r50.csv"
...
plot_grid <- rbind(c(1 , 1, NA, 3, 3, 3), c(2, 2, 2, 4, 4, 4))
grid.arrange(p1, p2, p3, p4, layout_matrix=plot_grid)
```

# Pareto opt
```{r, message=FALSE, fig.width=2.2, fig.height=1.9}
name <- "../data/exp4_d-2e-3_f12.csv"
results <- read_csv(paste(name, sep=""))

results %>% 
  # filter(errors < 0.6, variances > 0) %>%
  ggplot(aes(y=V_comp*1e3, x=V_osc*1e3, color=As)) +
  scale_color_gradient(low="mediumpurple", high="red3", name="A") +
  xlab("Vc (mvolt)") + ylab("Vo (mvolt)") +
  geom_point(alpha=0.4) + 
  theme_classic() + 
  theme(legend.position="bottom", legend.key.size =  unit(0.3, "in")) -> p1
...
results %>% 
  ggplot(aes(x=1e3*V_comp, y=errors)) +
  geom_point(alpha=0.4, size=.3, color="mediumpurple") +
  geom_smooth(method = "glm",  method.args = list(family = "binomial"), color="mediumpurple", alpha=0.2, se=FALSE, linetype="dashed") + 
  xlab("Vc (mvolt)") +
  ylab("Errors (s)") +
  theme_classic() -> p3
...
results %>% 
  ggplot(aes(x=1e3*V_osc, y=variances)) +
  geom_point(alpha=0.3, size=.3, color="red3") +
  geom_smooth(method = "glm",  method.args = list(family = "binomial"), color="red3", alpha=0.5, se=FALSE, linetype="dashed") + 
  xlab("Vo (mvolt)") +
  ylab("Variance (s)") +
  theme_classic() -> p4
...
plot_grid <- rbind(c(1, 1, NA, 2, 2, 2),c(1, 1, NA, 2, 2, 2),c(1, NA, NA, 2, 2, 2))
