...
fig2 %>% 
  filter(freq == 20) %>% 
  ggplot(aes(x=errors_pop*1e3, y=variances_pop*1e3, color=V_osc*1e3)) +
  geom_point(size=0.05, alpha=.4) +
  scale_color_gradient(low="white", high="red", name=expression(V[o] (mvolt))) +
...
fig2 %>% 
  filter(freq == 20) %>% 
  ggplot(aes(x=V_osc*1e3, y=errors_pop*1e3)) +
...
fig2 %>% 
  filter(freq == 20) %>% 
  ggplot(aes(x=V_osc*1e3, y=variances_pop*1e3)) +
  geom_point(size=0.05, alpha=1, color="black") + 
