...
fig1 %>% 
  ...
  mutate(V_free_initial = round(max(V_free), 3)) %>%
  filter(V_free_initial > 0) %>%
  ggplot(aes(x=V_osc/V_b, y=(variances/n_spikes)*1e3, group=N)) +
  geom_line(size=0.5, alpha=0.2, color="grey") + 
  theme_classic() +
  geom_line(data=filter(fig2, freq == 20), aes(x=V_osc/V_b, y=(variances_pop/n_spikes_pop)*1e3), 
            color="black", size=0.7, alpha=1) +
...
fig1 %>% 
  ...
  theme_classic() + theme(legend.position = c(1.25, 0.8), 
        ...
        legend.text = element_text(size=10)) -> p2
