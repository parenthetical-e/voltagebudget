...
fig1 %>% 
  ...
  filter(freq==20) %>% 
  filter(V_osc/V_comp <= 3) %>% 
  filter(N %in% sample(1:max_N, 18, replace = FALSE)) %>% 
  ggplot(aes(x=V_osc/V_comp, y=errors*1e3)) +
...
fig1 %>% 
  ...
  theme_classic() +
  labs(y=expression(V[o]/V[c]), x="Spike count") -> p4
...
rm(p1, p2, p3, p4, plot_grid, max_N)
```
...
plot_grid <- rbind(
  ...
  c(4,  4,  4,  4,  2,  2,  2, 2, 3, 3,   3, 3),
  c(4,  4,  4,  4,  2,  2,  2, 2, 3, 3,   3, 3),
  c(NA, NA, NA, NA, NA, NA, 5, 5, 5, 5,  NA, NA),
  c(NA, NA, NA, NA, NA, NA, 5, 5, 5, 5,  NA, NA),
  c(NA, NA, NA, NA, NA, NA, 5, 5, 5, 5,  NA, NA))
...
fig2 %>% 
  ...
  # scale_x_continuous(breaks=c(0, 4.0, 8.0, 12.0)) +
  theme_classic() +
...
fig2 %>% 
  ...
  geom_jitter(alpha=0.2, size=0.1) + 
  labs(x=expression(V[o]/V[c]), y="Error (ms)") +
  scale_color_gradient(low="gold", high="darkgreen") +
  theme_classic() +
  # scale_x_continuous(breaks=c(0, 4.0, 8.0, 12.0)) +
