...
```{r, fig.width=1.5, fig.height=2.0}
fig1 %>% 
  filter(rank == 0) %>% 
...
fig1 %>% 
  ...
  lims(y=c(0, 12)) +
  theme_classic() +
  scale_color_gradient(low="grey", high="black", name="Freq. (Hz)") +
...
fig1 %>% 
  ...
  labs(x="V_osc/V_comp", y="Error (ms)") -> p2
...
fig1 %>% 
  ...
  labs(x="V_osc/V_comp", y="Variance (ms)") -> p3
...
fig1 %>% 
  ...
  labs(x="V_osc/V_comp", y="Error (ms)") -> p4
