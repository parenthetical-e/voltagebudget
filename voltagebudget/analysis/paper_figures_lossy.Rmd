...
# Figure 8 - osc intro
```{r, fig.width=2, fig.height=1, message=FALSE}
# Load example spikes
ref_times <- read_csv("../../data/stim1.csv")
ref_times %>% 
   mutate(ts = ts - min(ts)) -> ref_times

# Make waves a function
time <- seq(0,0.4*2,length.out=1000)
freq <- 8
A <- 1
wave <- A/2 + A/2*sin(freq * time)

# As a test, plot an example of the wave
# plot(time, wave, xlab = "Time", ylab="Amplitude (namps)")

# Build a wave datafrome
amps <- c(
  min(vb_results$As[vb_results$As > 0]), 
  median(vb_results$As[vb_results$As > 0]), 
  max(vb_results$As[vb_results$As > 0]))

wave_results <- NULL
for (a in amps){
  df <- data.frame(time=time, 
                   wave=a*wave, 
                   As=rep(a, length(time)))
  wave_results <- bind_rows(wave_results, df)
}

# TODO - run simulations with As from above
...
p2 <- wave_results %>% 
  ggplot(aes(x=time, y=wave, color=As, group=As)) +
  geom_line() +
...
# HERE - need to run sims to make the osc_times df
osc_times <- NULL
p3 <- osc_times %>% 
  ggplot(aes(x=ts*1e3, y=ns, color=As)) +
  geom_point(alpha=0.8, size=0.6) +
  labs(x="Time (ms)", y="Neuron") +
  lims(x=c(0, 50)) +
  scale_color_continuous("Peak\namplitude (namps)", low="darkred", high="lightgoldenrod3") +
...
(p1 | p2) / p3 +
  ...
  plot_annotation(tag_levels = 'a', tag_suffix = ".")
ggsave("figures/lossy_fig8.png", dpi=600)
```

# Figure 9 - trade-off (with osc)
```{r, message=FALSE, fig.width=3.8, fig.height=1.8}
vb_results %>% 
...
vb_results %>% 
  ...
  lims(y=c(7,9)) +
  scale_color_continuous("Peak\namplitude (namps)", low="darkred", high="lightgoldenrod3") +
  ggtitle("Compression/error trade-off\nfor oscillation") +
  theme_classic() -> p3

((p1 / p2) | p3) +
  plot_layout(widths = c(1, 2)) + 
  plot_annotation(tag_levels = 'a', tag_suffix = ".")
ggsave("figures/lossy_fig9.png", dpi=600)
```


# Figure 10 - E50 intro
```{r, message=FALSE, fig.width=3, fig.height=2.8}
# Norm and annotate health versus path
vb_mean_results %>% 
  filter(V_osc/V_comp <= 5) %>%
  group_by(freq, N) %>%
  mutate(errors_pop = normalize(errors_pop)) %>%
  mutate(variances_pop = normalize(variances_pop)) -> vb_norm_results

vb_norm_results$state <- rep("pathological", nrow(vb_norm_results))
m <- vb_norm_results$errors_pop <= 0.5
vb_norm_results$state[m] <- "healthy"

# Create plots
vb_norm_results %>% 
  filter(freq == 8) %>% 
  ggplot(aes(x=As*1e9, normalize(errors_pop))) +
  geom_point(size=0.8, alpha=0.9) +
  labs(x="Peak\namplitude (namps)", y="Normalized\nError") +
  geom_hline(yintercept = 0.5, alpha=1, color="grey", linetype="dashed") +
  annotate("text", x = 0.1, y = 0.62, label = expression(paste("E"[50]))) +
  theme_classic() -> p1

vb_norm_results %>% 
  filter(freq == 8) %>% 
  ggplot(aes(x=As*1e9, normalize(variances_pop))) +
  geom_point(size=0.8, alpha=0.9) +
  labs(x="Peak\namplitude (namps)", y="Normalized\ncompression") +
  geom_vline(xintercept = 0.032, alpha=1, color="grey", linetype="dashed") +
  annotate("text", x = 0.046, y = 0.93, label = expression(paste("E"[50]))) +
  theme_classic() -> p2
...
vb_norm_results %>% 
  ...
  geom_hline(yintercept = 0.5, alpha=1, color="grey", linetype="dashed") +
  annotate("text", x = 1, y = 0.54, label = expression(paste("E"[50]))) +
  labs(y="Normalized\nError", x="Normalized\ncompression") +
  ggtitle("The pathological\ncompression hypothesis") +
  scale_color_manual("State", values=c("palevioletred3", "peachpuff3")) +
  theme_classic() +
  theme(legend.background = element_rect(colour ="black"),
        legend.position = "right") -> p3
...
(p1 | p2) / p3 +
  ...
  plot_layout(guides = 'collect') +
  plot_annotation(tag_levels = 'a', tag_suffix = ".") 
ggsave("figures/lossy_fig10.png", dpi=600)
```

# Figure 11 - VB intro
We don't have a reference, or there is no practical reference. Voltage budget analysis to the rescue!
```{r}
# HERE...
# Create VB intro
```

# Figure 12 - VB and E50
```{r}
# HERE
```
