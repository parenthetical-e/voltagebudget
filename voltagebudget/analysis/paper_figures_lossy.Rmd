...
p1 <- sync_times %>% 
  ggplot(aes(x=ts*1e3, y=ns)) +
  geom_point(alpha=0.8, size=1.2, shape=1) +
...
optimal_times %>% 
  ggplot(aes(x=ts*ms, y=ns)) +
  geom_point(size=1.2, alpha=0.8, shape=1) +
...
optimal_results %>% 
  ...
  labs(x="Error (ms)", y="Synchrony (ms)") -> p2
...
opt_v_osc_results %>% 
  ...
  scale_color_manual("Algorithm", 
                     values=c("darkslategray4", "darkseagreen3", "chartreuse3"),
                     guide = guide_legend(nrow = 3)) +
  labs(x="Synchrony (ms)", y="Error (ms)") +
