...
fig2 %>% 
  ...
  geom_point(size=.2, alpha=0.5) +
  geom_vline(xintercept = 0.27, alpha=1, color="darkslategray3", linetype="dashed") +
...
While the ratio determines the overall trade-off (above), again we see that about half the sync effect comes from weak oscillations (< 1 mV, orange) applied to strong inputs (i.e., V_c > 6 mV). However this story is complicated by the fact that the weaker Vc, the more sync is possible--though this sync is very high error. 
...
Fo far we've focused on error-variance relationships common between frequencies. There are also clearly frequency dependent effects 
