...
results %>% 
  ...
  ggplot(aes(x=communication_scores/max(communication_scores))) +
...
results %>% 
  filter(computation_scores < 0.6) %>%
  ggplot(aes(x=1-computation_scores/max(computation_scores))) +
  geom_histogram(bins=50) + 
  xlab("C") +
  theme_classic() -> p2
...
results %>% 
    filter(computation_scores < 0.5) %>%
  ggplot(aes(x=computation_voltages*1000, y=communication_voltages*1000, color=As)) +
  xlab("Vc (mvolts)") +
  ylab("Vo (mvolts)") +
  geom_point() + 
  theme_classic()
