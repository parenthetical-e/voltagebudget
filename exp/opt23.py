...
Search {A, a, b, Ereset} and maximizing {C, sigma_y}.
    ...
    Options:
        ...
        -a A                    maximum oscillation size [default: 2e-10]
...
if __name__ == "__main__":
    ...
    # ---------------------------------------------------------------------
    problem = Problem(M + 4, 2)
    problem.types[:] = [Real(0.0, Amax) for _ in range(M)] + [
        Real(0.0, (1 / f) * 0.5), Real(-1.0e-9, 1.0e-9), Real(
            10e-12, 60.0e-12), Real(-48e-3, -55e-3)
    ]
    ...
    As = [s.variables[:-4] for s in algorithm.result]
    ...
    header = ["C", "sigma_comp", "phis", "a", "b", "Ereset"]
