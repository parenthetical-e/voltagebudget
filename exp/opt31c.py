"""Usage: opt31c.py NAME N 
        (--lif | --adex)
        [-w W] [-a A] [-t T] [-f F] [-n N]

Search {phi, a, b, Ereset} and maximizing {Vcomp, Vosc}.
...
def create_problem(nrn, time, t_stim, N, ns, ts, f, A, Nz=100, **params):
    def problem(pars):
        ...
        print("opt: ({}, {}); par: (phi {}, a {}, b {}, Ereset {})".format(
            comp, osc, phi, a, b, E))
...
if __name__ == "__main__":
    ...
    sim = create_problem(nrn, t, t_stim, k, ns, ts, f, Amax, **params)

    # ---------------------------------------------------------------------
    problem = Problem(4, 2)
    problem.types[:] = [
        Real(0.0, (1 / f) * 0.5), Real(-1.0e-9, 1.0e-9),
    ...
    results = dict(
        ...
        phis=[s.variables[0] for s in algorithm.result],
        a=[s.variables[1] for s in algorithm.result],
        b=[s.variables[2] for s in algorithm.result],
        Ereset=[s.variables[3] for s in algorithm.result])
